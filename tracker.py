...
class HandTracker:
    ...
    def start(self):
        ...
        self.thread.start()
        print(f"--- Tracker Started (Thresholds: {self.min_ratio}-{self.max_ratio}) ---")
    ...
    def _calculate_logic(self, landmarks) -> Tuple[float, str]:
        ...
        if abs(dx) > abs(dy): return 0.0, "None" # Horizontal
        
        is_up = dy > 0
        orientation = "Up" if is_up else "Down"

        # 2. Reference Y & Elevation
        ref_y = (i_mcp.y + i_pip.y) / 2.0
    ...
    def _loop(self):
        ...
        # Optional: Set low resolution to save CPU
        ...
        with HandLandmarker.create_from_options(options) as landmarker:
            while self.running and cap.isOpened():
                ...
                # Smoothing
                alpha = 0.3 if detected else 0.5
                smooth_val = smooth_val + alpha * (target_pct - smooth_val)

                # Update State
                with self.lock:
                    self.percentage = smooth_val
                    self.is_detected = detected
                    self.hand_label = label
                    self.orientation = orient
                    
                    # Lazy Encoding: Only draw/encode if someone is watching
                    if self.active_clients > 0:
                        self._draw_debug(frame, draw_proto, smooth_val, orient, raw_ratio)
    ...
    def _draw_debug(self, frame, landmarks, val, orient, ratio):
        if not landmarks: 
